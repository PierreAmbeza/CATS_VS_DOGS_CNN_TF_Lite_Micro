...
#include "tensorflow/lite/micro/examples/cats_vs_dogs/model/cats_vs_dogs_model.h"
#include "tensorflow/lite/micro/examples/cats_vs_dogs/model/cats_vs_dogs_test_data.h"
...
int main(int argc, char* argv[]) {
  ...
  error_reporter->Report(
      ...
      cats_vs_dogs_model_tflite_len);
  const tflite::Model* model = ::tflite::GetModel(cats_vs_dogs_model_tflite);
  ...
  static const char* labels[] = {"Dog", "Cat"};
  ...
  const int tensor_arena_size = 1500 * 1024;
  ...
  int accurateCount = 0;
  const int inputTensorSize = 75*75*3;
  for (int s = 0; s < mnistSampleCount; ++s) {
    ...
    if (invoke_status != kTfLiteOk) {
      ...
      int guess =  get_top_prediction(model_output->data.i32, 2, error_reporter);
    
    error_reporter->Report("Model estimate [%s] training label [%s]",
                          labels[guess], labels[mnistOutput[s]]);
  ...
  error_reporter->Report("CATS vs DOGS classifier example completed successfully.\n");
