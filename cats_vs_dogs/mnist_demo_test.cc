...
TF_LITE_MICRO_TEST(TestInvoke) {
  ...
  TF_LITE_MICRO_EXPECT_EQ(75, model_input->dims->data[1]);
  TF_LITE_MICRO_EXPECT_EQ(75, model_input->dims->data[2]);
  TF_LITE_MICRO_EXPECT_EQ(3, model_input->dims->data[3]);
  ...
  for (int s = 0; s < mnistSampleCount; ++s) {
    ...
    for (int d = 0; d < 16875; ++d) model_input->data.f[d] = mnistInput[s][d];
    ...
    int max_score = model_output->data.i32[0];
    int guess = 0;

    for (int category_index = 1; category_index < num_categories;
         category_index++) {
      const int32_t category_score = model_output->data.i32[category_index];
